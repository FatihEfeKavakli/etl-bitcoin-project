...
def process_recent_files(**context):
    ...
    for obj in objects:
        ...
        for trade in trades:
            # Fiyat ve miktar: price/qty veya p/q olabilir
            try:
                price = float(trade.get("price") or trade.get("p"))
                qty = float(trade.get("qty") or trade.get("q"))
                value_usd = price * qty
            ...
            if value_usd > max_value_usd:
                max_value_usd = value_usd

            # Threshold: 1000 USD
            if value_usd < 1000.0:
                continue

            # Zaman: "time" veya "T" olabilir
            ts_ms = trade.get("time") or trade.get("T")
            if ts_ms is None:
                trade_time_iso = now.isoformat()
            else:
                try:
    ...
    print(
        f"DEBUG: Toplam incelenen trade sayısı = {total_trades}, "
        f"max value_usd = {max_value_usd}"
    )
    ...
    clean_key = (
        ...
        f"{now.hour:02d}/whales_{int(now.timestamp())}.json"
    )
    clean_bytes = json.dumps(whale_trades, ensure_ascii=False).encode("utf-8")

    if not MINIO_CLIENT.bucket_exists(CLEAN_BUCKET):
        MINIO_CLIENT.make_bucket(CLEAN_BUCKET)

    MINIO_CLIENT.put_object(
        bucket_name=CLEAN_BUCKET,
        object_name=clean_key,
        data=io.BytesIO(clean_bytes),
    ...
    for wt in whale_trades:
        cur.execute(
            """
            INSERT INTO whale_trades (symbol, side, price, qty, notional_usd, trade_time)
            VALUES (%s, %s, %s, %s, %s, %s);
            """,
            (
                wt["symbol"],      # symbol
                wt["side"],        # side
                wt["price"],       # price
                wt["quantity"],    # DB'de 'qty'
                wt["value_usd"],   # DB'de 'notional_usd'
                wt["trade_time"],  # trade_time
            ),
        )
